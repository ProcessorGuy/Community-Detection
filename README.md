# The Community Detection Algorithm Explained

### convertStringToLongLong.cpp
...
-  Basically, converts string to long long. Because int usually 4 bytes and we need more than 4 bytes because sometimes it might exceed the integer limits.
...
-  Basically, it splits the string/
...
- Basically, it opens the file and adds elements into data structure.
...
-  Basically, it writes the assigned communities into a file.
...
-  Basically, it calculates base ranks of the given edgeSet.

### calculateTotalRank.cpp
...
-  Basically, it calculates total ranks of the nodes from graph.
...
-  Basically, it assigns node into communities by using total rank of the nodes according to minimum size of the community size and removes overlapped nodes.
...
-  Basically, first it determines each unused nodes. Then, it creates counter table according to assigned communities frequencies. Then, it assigns nodes into communities and remove the used nodes from nodeSet. It iterates until used nodes size is zero.
